...
The thresholds values come in two degrees of severity: medium concern and strong concern. Different [operations](../operations) can be set depending on the degree of severity.

Some thresholds are defined in relation to some reference, as set by the reference data column. The [reference data](../reference_data) is defined separately. The values defined as thresholds therefore have to be multiplied with the values from the reference data before applying the criteria to scenario data.
